...
class ResultsDisplay:
    ...
    def _create_results_panels(self):
        ...
        self.quick_results_panel = pn.pane.HTML(
            ...
            <div style='color: #666; font-style: italic; padding: 20px; text-align: center;'>
                ðŸ“Š Analysis results will appear here...<br>
    ...
    def _format_basic_statistics_results(self, results: Dict[str, Any]):
        ...
        <div style='background: #F8F9FA; padding: 15px; border-radius: 6px; border: 1px solid #E0E0E0;'>
            ...
            <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;'>
                ...
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
                    <strong>Duration:</strong> {results.get('duration_mean', 0):.1f} Â± {results.get('duration_std', 0):.1f} s
                </div>
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
                    <strong>Voltage:</strong> {results.get('voltage_mean', 0):.3f} Â± {results.get('voltage_std', 0):.3f} V
                </div>
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
                    <strong>Capacity:</strong> {results.get('capacity_mean', 0):.4f} Â± {results.get('capacity_std', 0):.4f} Ah
                </div>
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
                    <strong>Energy:</strong> {results.get('energy_mean', 0):.4f} Â± {results.get('energy_std', 0):.4f} Wh
                </div>
            </div>

            <div style='color: #666; font-size: 12px; margin-top: 8px;'>
                <em>{data_source} â€¢ {results.get('analysis_timestamp', 'Current session')}</em>
            </div>
        </div>
        """
    ...
    def _format_resistance_results(self, results: Dict[str, Any]):
        ...
        if not resistance_analysis:
            ...
            <div style='background: #FFF3E0; padding: 15px; border-radius: 6px; border: 1px solid #F57C00;'>
                <div style='color: #F57C00; font-weight: 600; font-size: 16px; margin-bottom: 10px;'>
                    âš¡ Resistance Analysis Results
                </div>
        ...
        quality_indicator = (
            "ðŸŸ¢ Excellent" if quality_score >= 90 else
            "ðŸŸ¡ Good" if quality_score >= 70 else
            "ðŸ”´ Poor" if quality_score >= 50 else
            "âŒ Failed"
        )
        ...
        <div style='background: #FFF3E0; padding: 15px; border-radius: 6px; border: 1px solid #F57C00;'>
            ...
            <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;'>
                ...
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
                    <strong>Avg Resistance:</strong> {average_resistance:.4f} Î©
                </div>
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
                    <strong>Std Deviation:</strong> Â±{resistance_std:.4f} Î©
                </div>
            </div>

            <div style='color: #666; font-size: 12px; margin-top: 8px;'>
                <em>IR resistance analysis â€¢ Types measured: {', '.join(measurement_types)}</em>
            </div>
        </div>
        """
    ...
    def _format_dqdv_results(self, results: Dict[str, Any]):
        ...
        quality_indicator = "ðŸŸ¢ Excellent" if avg_r_squared > 0.9 else "ðŸŸ¡ Good" if avg_r_squared > 0.7 else "ðŸ”´ Poor"
        ...
        <div style='background: #E3F2FD; padding: 15px; border-radius: 6px; border: 1px solid #1976D2;'>
            ...
            <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;'>
                ...
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
                    <strong>Fit Quality:</strong> {quality_indicator} (RÂ²={avg_r_squared:.3f})
                </div>
                <div style='background: white; padding: 8px; border-radius: 4px; border: 1px solid #E8E8E8;'>
            ...
            <div style='color: #666; font-size: 12px; margin-top: 8px;'>
                <em>Electrochemical insights backend â€¢ Voltage relaxation analysis complete</em>
            </div>
        </div>
        """
    ...
    def _format_kinetics_results(self, results: Dict[str, Any]):
        ...
        completeness_indicator = (
            "ðŸŸ¢ Complete" if completeness_score == 100 else
            "ðŸŸ¡ Partial" if completeness_score >= 50 else
            "ðŸ”´ Limited"
        )
        ...
        <div style='background: #E8F5E8; padding: 10px; border-radius: 4px; border: 1px solid #2E7D32;'>
            ...
            <div style='color: #666; font-size: 10px; text-align: center;'>
                <em>ðŸ“Š Detailed kinetics analysis available in plot area below</em>
            </div>
        </div>
        """
