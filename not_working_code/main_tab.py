...
class DataAnalysisTab(param.Parameterized):
    ...
    def _create_context_bar(self):
        ...
        # Context bar layout
        return pn.Row(
            pn.pane.HTML("<strong>ðŸ“Š Data Analysis</strong>", margin=(10, 5)),
            pn.Spacer(width=20),
            self.cell_selector,
            pn.Spacer(width=20),
    ...
    def _create_analyze_button(self):
        ...
        self.analyze_btn = pn.widgets.Button(
            name="ðŸ” Analyze Selected Groups",
            button_type="primary",
            width=300,
            height=40,
    ...
    def _run_basic_statistics_analysis(self, selected_groups: List[str], settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for technique, segments in technique_groups.items():
                ...
                start_voltages = [seg.get('start_potential_v', 0) for seg in segments if
                                  seg.get('start_potential_v') is not None]
                end_voltages = [seg.get('end_potential_v', 0) for seg in segments if
                                seg.get('end_potential_v') is not None]
            ...
            print(
                f"  - Analyzed {len(all_segments)} segments across {len(technique_groups)} techniques: {', '.join(technique_groups.keys())}")

            # Keep overall totals for compatibility
            total_capacity = sum(
                seg.get('capacity_ah', 0) for seg in all_segments if seg.get('capacity_ah') is not None)
    ...
    def _run_resistance_analysis(self, selected_groups: List[str], settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"DEBUG: Resistance backend structure keys: {list(resistance_results.keys()) if isinstance(resistance_results, dict) else 'Not a dict'}")
            if isinstance(resistance_results, dict) and 'individual_resistances' in resistance_results:
                ...
                if individual_count > 0:
                    ...
                    print(
                        f"DEBUG: First resistance item keys: {list(first_item.keys()) if isinstance(first_item, dict) else 'Not a dict'}")
                    if isinstance(first_item, dict):
                        for key, value in first_item.items():
                            print(f"DEBUG: {key}: {value} (type: {type(value)})")
            else:
                print(
                    f"DEBUG: No 'individual_resistances' key found. Available keys: {list(resistance_results.keys()) if isinstance(resistance_results, dict) else 'None'}")
            ...
            if resistance_results and isinstance(resistance_results, dict):
                            ...
                            for segment_id, segment_data in group_data.items():
                                if isinstance(segment_data, dict) and 'current_pulse' in segment_data:
                                    ...
                                    if isinstance(pulse_data, dict):
                                        ...
                                        for key, time_point in [('immediate_resistance_ohm', 0),
                                                                ('resistance_10s_ohm', 10), ('resistance_30s_ohm', 30)]:
                                            if key in pulse_data and pulse_data[key] is not None and pulse_data[
                                                key] > 0:
                                                resistance_summary['resistance_values_ohm'].append(pulse_data[key])
                                                resistance_summary['time_points_s'].append(time_point)
                                                resistance_summary['measurement_types'].add(
                                                    key.replace('_resistance_ohm', '').replace('_ohm', ''))
                ...
                # Enhanced logging
                print(
                    f"Resistance analysis complete: {results['total_measurements']} total measurements (valid:{results['valid_measurements']}, null:{results['null_measurements']}), avg={results['avg_resistance']:.4f}Î©")
    ...
    def _run_kinetics_analysis(self, selected_groups: List[str], settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Debug: Print actual backend structure to understand data format
            print(
                f"DEBUG: Equilibrium backend structure keys: {list(equilibrium_results.keys()) if isinstance(equilibrium_results, dict) else 'Not a dict'}")
            if isinstance(equilibrium_results, dict):
                for key, value in equilibrium_results.items():
                    print(
                        f"DEBUG: {key}: {type(value)} = {value if not isinstance(value, (dict, list)) else f'{type(value)} with {len(value)} items'}")
            ...
            if equilibrium_results and isinstance(equilibrium_results, dict):
                ...
                if 'individual_equilibrium' in equilibrium_results:
                    ...
                    for equilibrium_data in individual_equilibrium:
                        if isinstance(equilibrium_data, dict):
                            ...
                            if 'voltage_infinity' in equilibrium_data and equilibrium_data[
                                'voltage_infinity'] is not None:
                            ...
                            if 'time_constant_s' in equilibrium_data and equilibrium_data[
                            ...
                            if 'diffusion_coefficient_cm2_s' in equilibrium_data and equilibrium_data[
                                'diffusion_coefficient_cm2_s'] is not None:
                                kinetics_summary['diffusion_coefficients'].append(
                                    equilibrium_data['diffusion_coefficient_cm2_s'])

                            # Track RÂ² quality from analytics config (direct field)
                ...
                print(
                    f"Kinetics analysis complete: {results['total_measurements']} total measurements ({quality_info})")
                print(
                    f"  - Equilibrium voltages: {eq_voltages}, Time constants: {time_constants}, Diffusion coeffs: {diffusion_coeffs}")
    ...
    def _update_status(self, message: str, status_type: str = "info"):
        ...
        icon = status_icons.get(status_type, "ðŸ“Š")
